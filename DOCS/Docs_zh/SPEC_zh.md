...
- 通过轻量级 Serving API 暴露稳定、可治理的分析指标
...
- **Serving API**：面向固定指标和管道健康状态的程序化访问接口
...
### 功能范围
Serving Layer 暴露的是**一组固定模板的分析端点**。它本质上是一个**只读、轻量的查询门面**，背后使用预定义查询模板，明确**不是**通用分析查询接口。
...
### 约束
- **仅支持固定模板端点**：每个端点都映射到针对特定 `agg_*` 或 `meta.*` 表的预定义查询
...
| 网络暴露 | 仅暴露本地 `localhost:8080`（Airflow UI）和 Serving API 本地端口 |
...
| 新鲜度 | 通过 meta 表与 Serving API 双重暴露 |
| 日志 | Airflow UI 日志 + Serving API 请求日志 |
...
3. **最后实现 Serving Layer**：完成 FastAPI 版本 Serving API，对接 marts 和 meta 表，并按验证标准逐项校验端点。
