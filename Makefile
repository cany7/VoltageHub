...
backfill:
	...
	docker compose exec $(AIRFLOW_SERVICE) airflow dags backfill $(AIRFLOW_DAG_ID) --start-date "$(START_DATE)" --end-date "$(END_DATE)"

dbt-deps:
	docker compose exec $(AIRFLOW_SERVICE) dbt deps --project-dir $(DBT_PROJECT_DIR) --profiles-dir $(DBT_PROFILES_DIR)

dbt-build: dbt-deps
	docker compose exec $(AIRFLOW_SERVICE) dbt build --project-dir $(DBT_PROJECT_DIR) --profiles-dir $(DBT_PROFILES_DIR) --target $(DBT_TARGET) $(DBT_VARS)

dbt-docs: dbt-deps
	docker compose exec $(AIRFLOW_SERVICE) dbt docs generate --project-dir $(DBT_PROJECT_DIR) --profiles-dir $(DBT_PROFILES_DIR) --target $(DBT_TARGET)

lint:
	uv run ruff check .
	set -a; . ./.env; export GCP_SERVICE_ACCOUNT_KEY_PATH=./keys/service-account.json; export GOOGLE_APPLICATION_CREDENTIALS=./keys/service-account.json; export UV_CACHE_DIR=/tmp/uv-cache; set +a; uv run --with 'dbt-core==1.8.*' --with 'dbt-bigquery==1.8.*' sqlfluff lint dbt/models
